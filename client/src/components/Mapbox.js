import React, { useRef, useEffect, useState } from 'react';
import './Mapbox.css';
import mapboxgl from 'mapbox-gl';
import 'mapbox-gl/dist/mapbox-gl.css';
import handleData from '../services/handleData.js';

mapboxgl.accessToken = window.env.REACT_APP_API_KEY_MAPBOX;

let processedData;


function Mapbox() {

  const mapContainer = useRef(null);
  const map = useRef(null);
  // Default coordinated of the map
  const [lng, setLng] = useState(-2.983333);
  const [lat, setLat] = useState(53.400002);
  // Default zoom
  const [zoom, setZoom] = useState(9);

  useEffect(() => {
    if (map.current) return; // initialize map only once
    map.current = new mapboxgl.Map({
    container: mapContainer.current,
    style: 'mapbox://styles/vadim-nest/clae2a9xr009514ozptbewmn9',
    center: [lng, lat],
    zoom: zoom
    });

    // Getting
    (async () => {
      console.log('Ok');
      processedData = await handleData();
      // ! YOU ARE HERE!!! ////////////////////////
      // ! At this point you have an array of objects including all of the fetched data, by location
      // console.log(processedData);
      console.log(processedData);
      displayHashtags(processedData, map);
      dynamicStyling();
    })();



  });

  return (
    <div>
      <div ref={mapContainer} className="map-container" />
      {/* <Hashtags /> */}
    </div>
  );
}

// async function processData() {
//   return await handleData();
// }

function displayHashtags (allData, map) {

  allData.forEach(array => {

    console.log(allData[3]);
    // console.log(array);
    for (let object of array) {
      if (object.hashtags && object.location.location_name !== 'global') {
        // Current location
        let lng = object.location.longitude;
        let lat = object.location.latitude;

        // The first paragraph of the first hashtag
        let theFirstParagraph = [object.urls[0].first_paragraph]
        console.log(theFirstParagraph);
        // the url of the



        object.hashtags.forEach((element, index) => {
          const el = document.createElement('div');
          el.className = `${object.location.location_name}Marker${index}`;
          el.classList.add(`${object.location.location_name}Marker`);

          let currentCoordinates = randomCoordinates(Number(object.location.longitude), Number(object.location.latitude), index);
          let allLinks = renderAllLinks(object.urls);

          // need to replace index below to all of the urls for the hashtag
          new mapboxgl.Marker(el)
              .setLngLat([currentCoordinates[0], currentCoordinates[1]])
              .setPopup(
                new mapboxgl.Popup({ offset: 25 }) // add popups
                  .setHTML(`
                    <h3>#${object.hashtags[index].hashtag}</h3>` + allLinks
                  )
              )
              .addTo(map.current);

          let h1 = document.createElement('H1');
          h1.innerText = `#${object.hashtags[index].hashtag}`;
          // h1.setAttribute('id', 'theMarker');
          h1.classList.add('theMarker');
          h1.classList.add('glow');

          let theMarker = document.getElementsByClassName(`${object.location.location_name}Marker${index}`);
          // console.log(theMarker);
          theMarker[0].appendChild(h1);

        });


      }


    }

  });
}

export default Mapbox;

function randomCoordinates(lng, lat, index) {

  if (index === 0) return [lng, lat - 0.001];

  // Big thanks to Isaac with helping with the maths here
  let angle = Math.random() * 2 * Math.PI;
  let radius = 0.005 * index;

  const newLat = lat + Math.sin(angle) * radius;
  const newLng = lng + Math.cos(angle) * radius;

  return [newLng, newLat];
}

function renderAllLinks(allUrls) {
  let ulStart = '<ul className-"articles">'
  let ulEnd = '</ul>'
  let finalReturn = allUrls.map(element => {
    return `<li><p>${element.first_paragraph}..</p><a target="_blank" href="${element.url}">Read more >></a></li>`
  })
  return (ulStart + finalReturn.join('') + ulEnd);
}

function dynamicStyling() {
  let differentFonts = [`'Inter', sans-serif`, `'Playfair Display', serif`, `'Raleway', sans-serif`, `'Roboto', sans-serif`, `'Sono', sans-serif`, `'Zilla Slab', serif`];
  let differentColors = ['#C2948A', '#FF7F11', '#FF1B1C', '#F58F29', '#A4B0F5', '#DBFF76', '#794D80', '#31CB00', '#45F0DF', '#F7DD72', '#F4ACB7']
  let differentColorsOnHover = ['#A06254', '#CC5F00', '#CC0000', '#C36609', '#4760EB', '#C3FF1F', '#482E4D', '#1D7A00', '#11D0BC', '#F2C618', '#EC6F81']


  let allHashtags = document.querySelectorAll('.theMarker');

  allHashtags.forEach(element => {
    // Set random fonts
    let randomFont = differentFonts[Math.floor(Math.random() * differentFonts.length)];
    element.style.fontFamily = randomFont;

    // Set random colours
    let colorIndex = Math.floor(Math.random() * differentColors.length);
    let randomColor = differentColors[colorIndex];
    let randomColorhover = differentColorsOnHover[colorIndex]
    element.style.color = randomColor;
    element.addEventListener('mouseover', function handleMouseover() {
      element.style.color = randomColorhover;
    })
    element.addEventListener('mouseout', function handleMouseover() {
      element.style.color = randomColor;
    })
  })
}

function hashtagSize(element) {

}


// ? Notes
// Displaying a marker
  // const src = 'data:image/png;base64,';
  // const el = document.createElement('div');
  // el.className = 'marker';
  // new mapboxgl.Marker(el)
  //     .setLngLat([-2.983333, 53.400002])
  //     .setPopup(
  //       new mapboxgl.Popup({ offset: 25 }) // add popups
  //         .setHTML(`
  //           <h3>Hello</h3>
  //           <p>World!</p>
  //           <a href="https://www.bbc.co.uk/news">link text</a>
  //           <img src=${src} alt="pic" />
  //           `
  //         )
  //     )
  //     .addTo(map.current);

  // let h1 = document.createElement('H1');
  // h1.innerText = "#Hashtag";
  // h1.setAttribute('id', 'theMarker');

  // let theMarker = document.getElementsByClassName('marker');
  // theMarker[0].appendChild(h1);
